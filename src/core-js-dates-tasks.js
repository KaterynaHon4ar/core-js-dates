...
function getTime(date) {
  const resultArr = [date.getHours(), date.getMinutes(), date.getSeconds()];
  return resultArr
    .map((el) => {
      if (el < 10) {
        return `0${el}`;
...
function getNextFriday(date) {
  ...
  if (currentDay < 5) {
    ...
    date.setDate(date.getDate() + (7 + 5 - currentDay));
  }
  return date;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const mSeconds = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  return Math.floor(mSeconds / 86400000) + 1;
}
...
function formatDate(date) {
  ...
  const hours24 = newDate.getUTCHours();
  ...
  return `${month}/${day}/${year}, ${hours12}:${minutes}:${seconds} ${amOrPm}`;
}
...
function getWeekNumberByDate(date) {
  ...
  newDate.setUTCDate(
    newDate.getUTCDate() + 3 - ((newDate.getUTCDay() + 6) % 7)
  );
  const firstThursdayOfTheYear = new Date(
    Date.UTC(newDate.getUTCFullYear(), 0, 4)
  );
  return (
    Math.round(
      (newDate.getTime() - firstThursdayOfTheYear.getTime()) / (7 * 86400000)
    ) + 1
  );
}
...
function getNextFridayThe13th(date) {
  ...
  let dateThe13th = new Date(year, month, 13);
  while (dateThe13th.getDay() !== 5) {
    month += 1;
    if (month > 11) {
      month = 0;
      year += 1;
    }
    dateThe13th = new Date(year, month, 13);
  }
  return dateThe13th;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const workDays = [];
  const dateToCheck = new Date(period.start.split('-').reverse().join('-'));
  const endDate = new Date(period.end.split('-').reverse().join('-'));
  while (dateToCheck <= endDate) {
    for (let i = 0; i < countWorkDays && dateToCheck <= endDate; i += 1) {
      workDays.push(
        dateToCheck.toLocaleDateString('en-GB').split('/').join('-')
      );
      dateToCheck.setDate(dateToCheck.getDate() + 1);
    }
    for (let i = 0; i < countOffDays && dateToCheck <= endDate; i += 1) {
      dateToCheck.setDate(dateToCheck.getDate() + 1);
    }
  }
  return workDays;
